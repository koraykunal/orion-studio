...
const stats = [
    { metric: "40+", label: "Projects shipped" },
    { metric: "12", label: "Industries served" },
    { metric: "98%", label: "Client retention" },
    { metric: "4.9", label: "Avg. review score" },
];
...
function ValueItem({
    ...
    return (
        <div ref={ref} className={`py-8 lg:py-10 ${!isLast ? "" : ""}`}>
            <svg
                viewBox="0 0 1000 2"
...
function StatItem({ stat, index }: { stat: (typeof stats)[number]; index: number }) {
    ...
    return (
        <div ref={ref} className="text-center lg:text-left">
            <span className="text-metric text-foreground block">{stat.metric}</span>
            <span className="text-label text-foreground-muted mt-3 block">{stat.label}</span>
        </div>
    );
}
